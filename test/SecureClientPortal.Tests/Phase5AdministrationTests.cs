...
public class Phase5AdministrationTests
{
    ...
    public async Task FirmManagementStoresTemplatesAndRules()
    {
        ...
        Assert.Equal(3, await db.RequiredDocumentTemplates.CountAsync());
        Assert.Single(await db.RequestTemplates.ToListAsync());
        Assert.Single(await db.ReminderRules.ToListAsync());
        Assert.Single(await db.DeadlineRules.ToListAsync());
        Assert.Single(await db.MonthlyPackTemplates.ToListAsync());
        Assert.Equal(2, await db.MonthlyPackTemplateItems.CountAsync());
        Assert.Contains(await db.MonthlyPackTemplateItems.Select(x => x.RequiredDocumentTemplateId).ToListAsync(), x => x == "rdt_bank_statement");
    ...
    public async Task RolesPersistPermissionCatalogAndLinks()
    {
        ...
        var permissions = await db.Permissions.OrderBy(x => x.Key).Select(x => x.Key).ToListAsync();
        Assert.Contains("documents.read", permissions);
        Assert.Contains("documents.review", permissions);

        var roleLinks = await db.RolePermissions.Where(x => x.RoleName == "review_manager").Select(x => x.PermissionKey).OrderBy(x => x).ToListAsync();
