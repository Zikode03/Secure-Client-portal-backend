...
using StorageFileStorage = SecureClientPortal.Backend.Application.Modules.Documents.IFileStorage;

namespace SecureClientPortal.Backend.Infrastructure.Modules.Documents.Application;
...
public sealed class DocumentCommandService : IDocumentCommandService
{
    ...
    public async Task<ServiceResult<Document>> CreateAsync(CreateDocumentRequest request, System.Security.Claims.ClaimsPrincipal user, CancellationToken ct = default)
    {
        ...
        switch (NormalizeDocumentStatus(request.Status))
        {
            ...
            case "rejected":
                created.Reject();
                slot?.RequestReupload(created.Id);
                break;
            case "filed":
                created.File(request.UploadedByUserId);
                break;
            case "submitted":
                slot?.Submit(request.UploadedByUserId, created.UploadedAtUtc);
                break;
        }
